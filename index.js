...
// Cache for tokens + last notification count
let expoTokens = [];
let lastTotalNotificationCount = 0;
...
const checkNotifications = async () => {
  try {
    console.log('Polling API for new notifications...');
    const response = await axios.get(
      'https://test-api.propfusion.io/notifications/?page=1&size=10',
      {
        headers: {
          Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6OTg4MCwicm9sZSI6InN1cGVyX2FkbWluIiwidHlwZSI6ImFnZW50IiwiZXhwIjoxNzUxMjE5MjE0fQ.VcnSbkewyAr1vLkAfv0AQfV54OkEmBlEgJkWBG3SUv0',
          Accept: 'application/json',
    ...
    const data = response.data;
    const newTotalNotificationCount = data.totalNotification;
    console.log(`New total notification count: ${newTotalNotificationCount}, Last total notification count: ${lastTotalNotificationCount}`);

    if (newTotalNotificationCount > lastTotalNotificationCount) {
      // Get the latest unseen lead notification
      const newLeadNotifications = data.notifications
        .filter(n => n.type === 'leads' && n.seen === false)
        .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); // Sort by timestamp descending

      if (newLeadNotifications.length > 0) {
        const latestNotification = newLeadNotifications[0]; // Get the most recent one
        console.log(`New lead detected, sending push notification for: ${latestNotification.subject}`);
        await sendPushNotifications(latestNotification);
      }

      lastTotalNotificationCount = newTotalNotificationCount; // Update last count
    } else {
